...
const { db, CalorieLogs } = require("../models");
...
router.post("/add", async (req, res, next) => {
  ...
  try {
    const created = await CalorieLogs.create({ email, amount, date });
...
router.put("/update", async (req, res, next) => {
  const { calorielogid, amount } = req.body;
  console.log(req.body);
  CalorieLogs.findOne({
    where: { calorielogid }
  })
